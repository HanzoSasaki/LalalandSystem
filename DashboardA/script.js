...
// Função para converter string de data em objeto Date (corrigida)
...
// Inicializar o datepicker com locale fixo
flatpickr(dateRangeInput, {
    ...
    defaultDate: ["01/07/2025", "14/07/2025"],
onChange: function(selectedDates) {
    if (selectedDates.length === 2) {
        // Garantir que as datas usem apenas dia/mês/ano
        ...
        // Atualizar texto de exibição (sem alterar comparação)
        const startStr = formatDateToDisplay(startDateSelected);
        const endStr = formatDateToDisplay(endDateSelected);
        dateDisplay.textContent = `Período selecionado: ${startStr} a ${endStr}`;
        applyFilters();
    }
}
});
...
function parseTsvData(tsv) {
    ...
    for (let i = 1; i < rows.length; i++) {
        ...
        // Formatar data para objeto Date
        rowData['DateObj'] = parseDate(rowData['DATA']);

        // Normalizar horário para meia-noite
        rowData['DateObj'].setHours(0, 0, 0, 0);
    ...
    // Definir datas padrão selecionadas
    startDateSelected = parseDate("01/07/2025");
    endDateSelected = parseDate("14/07/2025");
    endDateSelected.setHours(23, 59, 59, 999);
...
function applyFilters() {
    ...
    filteredData = allSalesData.filter(item => {
        // Filtro de data (usando objetos Date)
        if (startDateSelected && endDateSelected) {
            const itemDate = new Date(item.DateObj);

            // Remover informações de horário para comparação direta
            ...
            // Comparar apenas datas (ignorando horário)
            if (itemDateOnly < startDateSelected || itemDateOnly > endDateSelected) {
                return false;
            }
        }
    ...
    marginChart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                label: 'Margem Média (%)',
                data: avgPercentages,
                backgroundColor: 'rgba(213, 146, 240, 0.7)',
                borderColor: 'rgba(84, 19, 109, 1)',
                borderWidth: 1,
                type: 'line',
                yAxisID: 'y1'
            }]
